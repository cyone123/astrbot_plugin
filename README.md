...
| 配置项 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `cron_expression` | 字符串 (string) | `""` | Cron 定时检测表达式（如 `*/30 * * * *`），配置后**优先于轮询间隔生效** |
| `check_interval` | 整数 (int) | `1800` | 更新检查轮询间隔（秒），当未设置 Cron 表达式时生效，默认 30 分钟 |
...
### ⏰ Cron 表达式使用说明
本插件支持标准的 5 位 Cron 表达式（`分 时 日 月 周`），当配置了 `cron_expression` 时将优先按 Cron 时间表执行检查：
- `*/30 * * * *`：每 30 分钟检查一次（如整点 00 分、30 分）。
- `0 * * * *`：每小时整点检查一次。
- `0 */2 * * *`：每 2 小时整点检查一次。
- `0 8,12,18,22 * * *`：每天 08:00、12:00、18:00、22:00 定点检查。
- `0 0 * * *`：每天午夜 00:00 检查一次。

*注：若 `cron_expression` 留空或填写有误，将自动回退为 `check_interval` 间隔轮询模式。*
